...
We can create plots to inspect the performance of each subway line. The 3 "S" lines are excluded since these lines are missing a number of data. 
...
ggplot(temp, aes(x = Year, y = Passenger.Trips/1000000000)) +
  geom_line() +
  labs(title = "Unlinked Passenger Trips", y = "Billion Trips") +
  theme_minimal() +
  theme(plot.title = element_text(hjust=0.5), legend.title=element_blank()) +
  scale_x_continuous(breaks = seq(1992,2017, by = 5)) +
...
To tell if the expenses have fallen behind the rise in ridership, we have to normalize the expenses against the ridership. Here, we divide the expenses by the unlinked passenger trips. It is a better measurement to tell if the rise in expenses have kept up with the rise in ridership. From this point on, we will use the normalized metrics, all obtained by dividing the original data by the unlinked passenger trips corresponding to the data.
...
In this section, the graphs we constructed seem to suggest that there are some questionable trends in NYCT's spending decisions. But are these fund allocations affecting the performance? In the next section, we will attempt to answer this question.


### Solutions Through Linear Modeling

We now move forward to ask this question, "Can the NYCT's spending decision affect maintenance performance?" Here, we propose a frame work to test and check if the spending has any relationship with the performance. Namely, we will create a number of linear regression models, using normalized mechanical failures as the target response variable, and the normalized expenses as the explanatory variables. We will check to see if the slope of the linear line is significant. Using an alpha level of 0.05, we check if the p-value is less than the alpha level, so we can reject the null hypothesis that the slope is zero. A slope of zero means that there is no relationship between the expense and the mechanical failures.  

Since the mechanical failures data is only available from 1997 to 2016, any explanatory variables data prior to 1997 are excluded. Also, to simplify the analysis, we sum up the major and minor failure types to create a new variable `Total.Failures`. 
...
Below is a scatter plot matrix, pairing up the capital expenses with the new failure variable. In the upper panel of the matrix, we calculate the Pearson Correlation Coefficients between the variables. In the diagonal panel, the histograms for the variables are shown.
...
The plot matrix shows that the `Total.Failures` has a high correlation with `Other.Cap`. Next, we create a function that can perform the linear regression and construct a summary table. We can use this function repeatedly.
...
The table shows that only the `Other.Cap` variable has a slope that has a p-value lower than the alpha level of 0.05. We can plot the scatter plot and the linear model line below. The negative slope suggests that increase the `Other.Cap` spending will lower the number of mechanical failures.
...
How this help reduces the total failure is not clear. This information is not helpful until after taking a deeper look at the breakdown of the `Other.Cap`. 

For now, let us turn to the operating and maintenance expenses. We again begin with a scatter plot matrix, follow by a table summarizing the estimates of the linear models.
...
In particular, the slope for `Vehicle.Maint` has a steeper slope, suggesting that it has a larger effect on reducing the number of mechanical failures. Moreover, the linear model puts a figure on how much we can reduce the total mechanical failures. The slope for `Vehicle.Maint` suggests that for each additional 1 million USD spend on vehicle maintenance, NYCT would reduce the number of mechanical failures by `r round(abs(expense_lm$Slope[3]))`.  
...
There are few interesting discoveries. The `Services` expense category, which are the payments made to 3rd party contractors, has a positive slope. Without further research, we can only speculate that this may be because more mechanical breakdown leads to more 3rd party services being ordered to repair or supplement the service. The relatively small adjusted R squared for this variable also suggests that the variable only explains a small portion of the variation.
...
In summary, the linear models constructed above suggest that the spending increases in the following expense categories, ranked in the order of effectiveness, can reduce the number of mechanical failures in the system, which may lead to service improvement:
